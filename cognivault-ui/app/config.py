...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "rag": {
        ...
        # Поводки скрытых вызовов. Дефолты подобраны под стриминговый GigaChat;
        # для KitAI, который ставит запрос в очередь и опрашивает результат, их
        # приходится поднимать — иначе грейдер отваливается по таймауту на
        # каждом ходе и отбор фрагментов не работает вовсе.
        "condense_timeout": 10.0,
        "grader_timeout": 20.0,
        "grader_enabled": True,
        "grader_threshold": 4,
