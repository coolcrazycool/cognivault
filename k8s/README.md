...
> Он полный (бэкенд + UI + Service + Ingress + egress) и с актуальными пинами образов.
> Разворачивайте окружение по нему.
>
> Файлы в `k8s/` оставлены **для истории и отката**; в них нет манифестов UI. Пин
> образа бэкенда держится вровень с `deploy/dropapp/` — иначе «откат» означал бы
> выкатку случайной старой сборки, а не предыдущего релиза. Устарело и описание внешнего Qdrant ниже: Platform V
> Vector DB подключается по mTLS с JWT от IAM (`QDRANT_USERNAME`/`QDRANT_PASSWORD`
> обмениваются на Bearer-токен), а не по Basic-аутентификации. Ничего отсюда не
> применяйте, не сверившись с `deploy/dropapp/`.
