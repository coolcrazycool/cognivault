...
```json
{"id": "3f2a9c1b0d-f", "question": "Какой overlap у чанков?", "ground_truth": "Overlap отсутствует.", "kind": "factual", "source_path": "docs/indexing.md", "section_path": "Индексация > Чанкер", "source_chunk_index": null, "expected_refusal": false, "accepted": null}
```
...
```json
{
  ...
  "dispersion": {"faithfulness_ru": {"mean": 0.71, "sd": 0.24, "n": 78, "stderr": 0.027}},
...
правило диагностики, таблица по парам (с колонкой «чанк найден») и список
упавших пар.
...
* Диф предупреждает, если прогоны сделаны разными версиями судейских промптов,
  на разных golden-set, с разными параметрами прогона или если хотя бы один из
  них **приближённый**.
...
разбросом и числом пар, ветка отказа.
