...
1. **Заголовочных метрик тринадцать, а не сорок.** Из каждого стыка берётся то,
...
2. **Метрики делятся на ДЕТЕРМИНИРОВАННЫЕ и ВЫБОРОЧНЫЕ.** Стыки 1–2 считаются по
   ...
   это ±0.036, на 160 сгенерированных — ±0.006. Мерить их одной линейкой значит
   либо утопить приёмочную регрессию в «шуме», либо объявить изменением дрожание
   одного вопроса.
...
def headline(reports: Mapping[str, Mapping[str, Any]]) -> dict[str, Measurement]:
    ...
    for origin in ("customer", "generated"):
