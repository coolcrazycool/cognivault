...
describe('windowFor', () => {
  ...
  it('сообщает о ненайденном якоре, а не притворяется, что нашёл', () => {
    // Табличный чанк несёт крошку `${sectionPath} > Таблица`, которой в разделе нет:
    // `chunkBody` снимает только `${sectionPath}\n\n`, и чанк перестаёт находиться.
    // Мост обязан показать это флагом, иначе тихий фолбэк в префикс не измерить.
    const marker = 'строки таблицы, которые ищутся в разделе';
    const section = longSection('Раздел', marker, 'хвост');
    const result = windowFor(internals, section, {
      section: 's',
      chunk_text: `Раздел > Таблица\n\n${marker}`,
      section_path: 'Раздел',
      limit: 1000,
      mode: 'centred',
    });
