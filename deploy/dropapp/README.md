...
|-----------|-----------------|-------------------|
| бэкенд | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault:sha-9d1b862` | этот релиз |
| UI | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault-ui:sha-cd09d56` | этот релиз |

> **Оба тега собраны и проверены в реестре.** Они разные, и это нормально: воркфлоу
> бэкенда и UI фильтруются по путям, а последний коммит релиза правил только `src/` и
> README — сборка UI на него не запускалась и осталась на предыдущем коммите той же
> ветки. **Не пытайтесь «подтянуть» один тег к другому**: образа с выдуманным тегом не
