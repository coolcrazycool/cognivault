...
> `cognivault-ui/`, поэтому оба воркфлоу собрались с него. Расходиться они тоже
> могут — фильтры по путям, и компонент, который не менялся, остаётся на прежнем
> коммите. **Не пытайтесь «подтянуть» один тег к другому**: образа с выдуманным тегом не
...
>
> ⚠️ **Бэкенд катите НЕ ПОЗЖЕ UI.** Дерево разделов в ответах читается из
> `GET /api/vault/catalog`, который появился в этом же релизе бэкенда. Новый UI на
> старом бэкенде молча отдаёт ответы без головного блока — ни дерева, ни «состава
> базы», и никакой ошибки при этом не будет.
>
> ⚠️ **После выкатки бэкенда обязательно пройдите раздел 3.7.** Этот релиз меняет схему
> разреженных векторов, и старый индекс надо пересоздать — без этого поиск по словам
> молча работает хуже, ошибки нигде не будет.
>
...
2. новые ключи в ConfigMap'ах, если релиз их добавил. **Этот релиз их не добавил.**
...
6. **Проверить схему индекса и, если она устарела, пересоздать коллекцию** —
   настройки UI → «Индекс и поиск», раздел 3.7. Пропустить этот шаг легко: ничего
   не падает, поиск отвечает, просто хуже. **На этом релизе он обязателен** —
   схема разреженных векторов сменилась (v2 → v3), и все сохранённые векторы
   устарели. Если в том же разделе висит красное «индекс заблокирован» — это не
   «хуже», а «никак»: пересоздание обязательно, и подтверждать надо `cognivault`.
...
- **меняется схема разреженных векторов (`BM25`)** — тогда нужна не переиндексация,
  ...
  которая коллекцию создала, а персональный reindex её не трогает. В этом релизе
  это ровно тот случай (v2 → v3), см. 3.7;
