...
| UI | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault-ui:sha-738c195` | этот релиз |
...
### Что нового в этом релизе (бэкенд `sha-d1d5cd6`, UI `sha-738c195`)
...
| `CHAT_PROVIDER` | **`kitai`** | транспорт чата; `gigachat` — откат на прямой mTLS-клиент со стримингом |
...
| `KITAI_MODEL` | `glm-5.2` | модель генерации ответа на KitAI |
