...
|-----------|-----------------|-------------------|
| бэкенд | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault:sha-b5d3c60` | **предыдущий** релиз |
| UI | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault-ui:sha-977445a` | **предыдущий** релиз |

> ⛔ **Образов этого релиза пока НЕТ, и выкатывать его нечем.** Ветка с изменениями
> (конвертер, чанкер, BM25 v3, пересоздание коллекции из UI) в `origin` ещё не
> запушена, поэтому ни один сборочный прогон по ней не запускался. Самое свежее, что
> реально лежит в реестре на момент написания: бэкенд `sha-06c45bf` (и совпадающий
> с ним по содержимому `sha-b5d3c60`), UI `sha-977445a` — **обе сборки старше всех
> изменений этого релиза**. Пины в `04-backend.yaml` и `05-ui.yaml` намеренно
> оставлены на них: подставлять несуществующий тег хуже, чем ехать на предыдущей
> версии.
>
> **Что делать оператору.** Не выдумывать тег и не ставить `latest` (SberOSC его не
> проксирует). Порядок такой:
>
> 1. дождаться, пока разработчик запушит ветку и оба воркфлоу (**Build and Publish
>    Docker Image** и **Build and Publish CogniVault UI Docker Image**) отработают
>    зелёными;
> 2. взять теги из фактических сборок — страницы пакетов
>    `github.com/coolcrazycool/cognivault/pkgs/container/cognivault` и
>    `…/cognivault-ui`, либо коммит успешного прогона в **Actions** (тег =
>    `sha-` + первые 7 символов его sha);
> 3. подставить оба тега в `image:` соответствующих Deployment'ов (раздел 8) —
>    ровно те, что есть в реестре, даже если они не совпадают между собой;
> 4. после выкатки бэкенда обязательно пройти раздел 3.7 — этот релиз меняет схему
>    разреженных векторов, и старый индекс надо пересоздать.
>
> Проверить, что тег вообще существует, можно и не выходя из кластера: если образа
> нет, под встанет в `ImagePullBackOff` с `manifest unknown` — это не проблема
...
#### Нужно ли пересоздавать — как отличить два случая
...
> поэтому он вынесен в процедуру отдельным пунктом.
...
4. Морда покажет предупреждение и попросит **ввести название коллекции руками**
   (`cognivault_v2` — точное имя показано прямо в форме, поле намеренно не
   заполняется автоматически). Кнопка «Пересоздать» разблокируется только при
   точном совпадении.
...
- строка состояния перечитывается сама, и предупреждения про устаревшую схему
  больше нет;
...
| 13 | **Посмотреть состояние индекса**: настройки UI → «Индекс и поиск». Есть предупреждение про устаревшую схему или нет? | браузер | 3.7 |
| 14 | По итогу шага 13 — **одно из двух**: схема актуальна (обычный случай чистого старта) → полная переиндексация; схема устарела → **«Пересоздать коллекцию»**, она же переиндексирует всех, отдельная переиндексация после неё не нужна | браузер | 3.7, 6.3 |
...
| `uses the legacy UNNAMED vector schema` / `has no "dense" vector` | коллекция, на которую смотрит алиас, создана старой схемой | **под не поднимается, поэтому кнопка из UI недоступна** — удалять коллекцию руками, раздел 3.6, потом переиндексировать (6.3) |
...
> переиндексация до него — выброшенное время.
