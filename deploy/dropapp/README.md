...
| UI | `sberosc.sigma.sbrf.ru/ghcr.io/coolcrazycool/cognivault-ui:sha-a83e751` | этот релиз |
...
### Что нового в этом релизе (бэкенд `sha-d1d5cd6`, UI `sha-a83e751`)

> **Релиз 2026-09-02 (UI `sha-a83e751`, откат — `sha-9bb9bca`).** Бэкенд не менялся.
> Вместе с образом импортировать `03-configmap-ui.yaml`: новые ключи
> `RAG_GRADER_MAX_TOKENS`, `RAG_CONDENSE_MAX_TOKENS`, `UI_LOG_LEVEL`, закомментированный
> пример `KITAI_EXTRA_BODY`. **`KITAI_MODEL: glm-5.1` на платформе больше не существует**
> (404 «No such model», 2026-09-02): либо впишите живое имя сюда, либо после каждого
> перезапуска пода выставляйте модель в UI заново — `config.json` живёт на `emptyDir`.
> Что внутри: исход каждого скрытого вызова (грейдер, condense) пишется в
> `rag_log.jsonl` и показывается в отчёте оценки с причиной; бюджеты вывода скрытых
> вызовов 4096/2048 вместо зашитых 1024/512; повтор при пустом ответе по лимиту и
> уведомление пользователю; прогон оценки больше не глотает лог пода, его буфер лежит в
> `eval-<label>.log`; судья харнесса v2 (прежние отчёты несопоставимы). Подробно —
> `HANDOFF.md` §0.
...
| `KITAI_MODEL` | `glm-5.1` | правится без пересборки. Проверено: `glm-5.2` на этом контуре не роутится; **с 2026-09-02 и `glm-5.1` отвечает 404 «No such model»** — впишите живое имя или выставляйте модель в UI после каждого перезапуска |
...
| `RAG_INCLUDE_ARCHIVED` | `true` | искать И по архивным страницам. Бэкенд по умолчанию вырезает всё под папкой «Архив» и всё с `archived:` во frontmatter; на живом дереве под Архивом оказался раздел с актуальными страницами, и за весь прогон `baseline` оттуда не пришло ни одного источника из 215 — четыре «промаха ретрива» оказались этим фильтром. `false` возвращает прежнее поведение |
| `RAG_GRADER_THRESHOLD` | `4` | порог оценки 1–5 |
| `RAG_GRADER_KEEP_TOP` | `1` | одна страховочная позиция по сырому рангу и только если судья её не забраковал (оценка ≥ 3). Было 2 (проезжали оценки 1), потом 0 — но тогда одна ошибка судьи выбивала топ-1 поиска |
| `RAG_CONDENSE_TIMEOUT` | `45` | поводок вызова 1. Совпадает с дефолтом в коде (был 10 с — под стриминговый GigaChat) |
| `RAG_GRADER_TIMEOUT` | `90` | поводок грейдера. Совпадает с дефолтом в коде. Прежние 20 с резали по медиане: в прогоне `baseline` грейдер упёрся в них на 41 ходе из 46, вернул оценки на 2 парах из 44, и контекст собирался сырым порядком поиска — молча, без единой ошибки |
| `RAG_RERANK_CANDIDATES` | `40` | ширина ретрива (Волна 3: было 20) |
| `RAG_GRADER_MAX_TOKENS` | `4096` | бюджет ВЫВОДА одного батча грейдера (был зашит 1024). Рассуждающая модель тратит `max_tokens` сначала на рассуждения; при 1024 на JSON не оставалось ничего — «GigaChat вернул пустой ответ», реранкер мёртв на каждом ходу. Совпадает с дефолтом кода |
| `RAG_CONDENSE_MAX_TOKENS` | `2048` | то же для вызова 1 (был зашит 512) |
| `KITAI_EXTRA_BODY` | — | JSON-объект, ключи которого добавляются в запрос к KitAI, если их там ещё нет: например `{"enable_thinking": false}`, чтобы выключить режим рассуждений без пересборки. Кривой JSON → предупреждение и игнор |
| `UI_LOG_LEVEL` | `INFO` | уровень логгеров `cognivault-ui.*` в логе пода. До этого `log.info` не печатался вообще, а на время прогона оценки терялись и предупреждения |
