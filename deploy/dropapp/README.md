...
### Что нового в этом релизе (`sha-8955d97`)

> **Переиндексация и пересоздание коллекции НЕ нужны.** Этот релиз не меняет ни схему
> коллекции, ни версию BM25, ни схему SQLite и не добавляет ни одного ключа ConfigMap —
> сверено по диффу `src/config.ts`, `cognivault-ui/app/settings.py`, `src/db/`,
> `drizzle/`, `src/lib/bm25.ts`. Выкатка — это только замена двух образов. Раздел 3.7
> нужен, **только** если вы обновляетесь с версии старше `sha-8665e36`.
...
6. **Проверить схему индекса и, если она устарела, пересоздать коллекцию** —
   ...
   не падает, поиск отвечает, просто хуже. **На релизе `sha-8955d97` он НЕ нужен** —
   схема не менялась; он обязателен только при переходе с версии старше `sha-8665e36`,
   где сменилась схема разреженных векторов (v2 → v3). Если в том же разделе висит
   красное «индекс заблокирован» — это не «хуже», а «никак»: пересоздание
   обязательно в любом случае, и подтверждать надо `cognivault`.
...
> бэкенда маршрута, которого в развёрнутом бэкенде нет.** Для `sha-8955d97` это условие
> выполнено — новых обращений к бэкенду в UI не появилось, а `GET /api/vault/catalog`,
> от которого зависит дерево разделов, приехал ещё в `sha-8665e36`. Проверить, что
> дерево живо, можно одним вопросом в свежем чате: «О чём эта база?» — ответ со
> структурой и **без источников** означает, что каталог доступен.
...
- **меняется схема разреженных векторов (`BM25`)** — тогда нужна не переиндексация,
  ...
  `sha-8665e36` (v2 → v3), см. 3.7; в `sha-8955d97` схема не менялась;
